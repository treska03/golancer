...
[README](README.md) for what it _does_ do today.

## Reliability

- [ X ] **Health checking.** The pool currently trusts that every registered
...
- [ X ] **Retries / failover.** If a chosen backend fails mid-request, try the
  next one instead of surfacing the error to the client.
- [ X ] **Graceful shutdown.** `cmd/balancer` calls `http.ListenAndServe`
